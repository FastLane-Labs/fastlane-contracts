...
import { Storage } from "../../src/atlas/core/Storage.sol";
...
contract StorageTest is AtlasBaseTest {
    ...
    function test_storage_transient_lock() public {
        ...
        TestAtlas(address(atlas)).setLock(address(1), 2, 3);
        ...
        TestAtlas(address(atlas)).clearTransientStorage();
        (activeEnv, callConfig, phase) = atlas.lock();
    ...
    function test_storage_transient_isUnlocked() public {
        ...
        TestAtlas(address(atlas)).setLock(address(1), 0, 0);
    ...
    function test_storage_transient_solverLockData() public {
        ...
        TestAtlas(address(atlas)).setSolverLock(testSolverLock);
...
// Additional functions added to TestAtlas helper for transient storage
interface TestAtlas {
    function setLock(address activeEnvironment, uint32 callConfig, uint8 phase) external;
    function setSolverLock(uint256 newSolverLock) external;
    function clearTransientStorage() external;
}