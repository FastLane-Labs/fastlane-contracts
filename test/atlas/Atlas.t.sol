...
contract AtlasTest is BaseTest {
    ...
    function test_Atlas_BundlerRefundedIfMetacallFails() public {
        ...
        bool success = atlas.metacall{ value: 1e18 }(userOp, new SolverOperation[](0), dAppOp, userEOA);

        // Metacall should fail, return false instead of reverting, and refund the bundler
        assertEq(success, false, "Metacall should fail");
        assertEq(address(userEOA).balance, bundlerBalanceBefore, "Bundler should be refunded, no balance change");
    }
...
contract TestDAppControl is DAppControl {
    ...
    function _allocateValueCall(bool solved, address bidToken, uint256 bidAmount, bytes calldata data)
        internal
        virtual
        override
    { }
