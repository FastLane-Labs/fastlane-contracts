...
abstract contract Escrow is AtlETH {
        ...
        returns (uint256)
    {
            ...
            if (_result.canExecute()) {
                ...
                if (_result.executionSuccessful()) {
                    ...
                    if (dConfig.callConfig.multipleSuccessfulSolvers()) {
                        ...
                        _handleSolverFailAccounting(solverOp, dConfig.solverGasLimit, _result, false);
                    } else {
        ...
        _handleSolverFailAccounting(solverOp, dConfig.solverGasLimit, _result, dConfig.callConfig.exPostBids());
        ...
        view
    {
        ...
        // NOTE: On Monad, unreachedSolverGas is the sum of solverOp.gas, not dConfig.solverGasLimit
        ...
        returns (uint256, uint256 gasLimit)
    {
        ...
        // Get solver's bonded shMON amount (measured in MON terms) to check if enough to pay for execution
        uint256 _solverBalance = SHMONAD.convertToAssets(SHMONAD.balanceOfBonded(POLICY_ID, solverOp.from));
