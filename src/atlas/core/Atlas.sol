...
contract Atlas is Escrow, Factory {
    ...
    /// @dev Any MON sent as msg.value with a metacall should be considered a potential subsidy for the winning solver's
    /// gas repayment.
        ...
        returns (bool auctionWon)
    {
        ...
        uint256 _gasMarker = _gasLeft + _BASE_TX_GAS_USED;
        // NOTE: On Monad, winning solver pays for the full gas limit of the metacall (excl. writeoffs and gas due to
        // other solvers), and not just the gas used. As such, we do not add the usual _FIXED_GAS_OFFSET here, as that
        // represents gas which will still be used after the gasleft() snapshot is taken in `_settle()`.
        ...
        try this.execute{ gas: _gasLimit }(
            ...
            if (!_dConfig.callConfig.exPostBids()) {
                _unreachedCalldataValuePaid = _chargeUnreachedSolvers(
                    solverOps,
                    _gL,
                    ctx.solverIndex,
                    dAppOp.userOpHash,
            ...
            // WARNING: If msg.sender is a disposable address such as a session key, make sure to remove MON from it
            // before disposal
            if (msg.value != 0) SafeTransferLib.safeTransferETH(msg.sender, msg.value);
        ...
        returns (uint256)
    {
        ...
        for (; ctx.solverIndex < solverOpsLen; ctx.solverIndex++) {
            ...
            if (dConfig.callConfig.multipleSuccessfulSolvers()) {
                // Considered a fail for simulation purposes when only one solverOp in the metacall, and it fails.
                // If more than 1 solverOp, any of them could fail and simulation could still be successful.
